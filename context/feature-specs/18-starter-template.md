...
4. Wire starter templates into the editor.
   ...
   - when a template is selected, add its nodes and edges alongside whatever is already on the canvas
   - position the imported nodes so they don't overlap existing content instead of clearing or replacing it
...
- Import action adds the template's nodes and edges alongside existing canvas content through the existing node and edge state flow.
